...
function CameraController({
  ...
  useFrame(({ camera }, delta) => {
    if (!animStarted) {
      ...
      camera.position.z = THREE.MathUtils.clamp(camera.position.z, clampZMin, clampZMax)
      return
    }
...
function LandingScene({
  ...
  // ✅ IMPORTANT: clamp uses introTargetZ
  const { clampZMin, clampZMax } = useMemo(() => {
    const minZ = Math.min(CAMERA_CONFIG.INTRO_START_Z, introTargetZ)
    const maxZ = Math.max(CAMERA_CONFIG.INTRO_START_Z, introTargetZ)
  ...
  return (
    <>
      ...
      <Selection>
        ...
        <CyberpunkSkyline position={[0, -1, -20]} girlTextureUrl="/videos/girl_pha_v2.png" />
        ...
        <group position={[-1, 0, -0.5]}>
          ...
          <spotLight position={[-3, 1.5, 2]} intensity={1.15} angle={0.6} penumbra={0.8} />
...
function SectionReveal({
  ...
  useEffect(() => {
    // whenever section becomes active: hide, then show after delay
    ...
    setShow(false)
    tRef.current = window.setTimeout(() => setShow(true), delayMs)
...
export default function DesktopPage() {
  ...
  // ✅ declare active ONCE, BEFORE any effects that use it
  ...
  const [showreelTriggered, setShowreelTriggered] = useState(false)
  ...
  // showreel trigger
  useEffect(() => {
    if (showreelTriggered) return
    if (!userInteracted) return
    if (active === SHOWREEL_INDEX) setShowreelTriggered(true)
  }, [active, userInteracted, showreelTriggered])
  ...
  // intersection observer sets active
  useEffect(() => {
    const main = mainRef.current
    if (!main) return
    const els = ids
  ...
  // main scene fade + pointer gating (original behaviour)
    ...
    <div style={{ width: "100vw", height: "100svh", position: "relative", overflow: "hidden" }}>
      ...
      >
        ...
        {sectionData.map((section, idx) => {
          const i = idx + 1
          const isActiveSection = active === i
          ...
          if (isWorkSection) {
            ...
            content = <VideoModelTexture rotate={active === SHOWREEL_INDEX} />
          } else if (isArtSection) {
             content = <ShaderFrame isMobile={isMobile} headerHeight="64px" />
            ...
            content = (
              ...
              content = <ShaderFrame isMobile={isMobile} headerHeight="0px" />
          ...
          return (
            ...
            >
              <SectionReveal active={active === i} delayMs={1000}>
                {content}
              </SectionReveal>
            </section>
          )
