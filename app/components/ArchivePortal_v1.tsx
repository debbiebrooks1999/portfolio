...
export default function ArchivePortal({ activeSection }: ArchivePortalProps) {
  ...
  return (
    <div className="relative w-full h-[85vh] overflow-hidden bg-[#050609] text-slate-100 font-sans">
      {/* Content container */}
      <div className="relative z-10 w-full max-w-[90rem] mx-auto h-full min-h-0 px-2 sm:px-3 md:px-6 py-2 flex flex-col gap-2">
        {/* Header */}
        <div className="shrink-0 backdrop-blur-xl bg-[#060810]/80 border border-white/5 rounded-2xl md:rounded-3xl px-3 sm:px-4 md:px-5 py-2.5 md:py-3 shadow-[0_18px_45px_rgba(0,0,0,0.65)]">
        ...
        {/* Footer */}
        <div className="shrink-0 backdrop-blur-xl bg-[#05070c]/80 border border-white/5 rounded-2xl md:rounded-3xl px-3 sm:px-5 py-3 text-center text-[11px] md:text-xs text-slate-500">
...
function FilterPill({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`px-3 sm:px-3.5 py-2.5 sm:py-2 rounded-full text-sm sm:text-xs font-medium transition-all ${
        active
          ? "bg-sky-500 text-slate-950 shadow-[0_0_20px_rgba(14,165,233,0.3)]"
          : "bg-[#0b1018] text-slate-300 border border-white/10 hover:border-white/25 active:border-white/40"
      }`}
    >
      {label}
      {count !== undefined && (
...
function ProjectCard({
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-xl border border-white/8 bg-[#070910]/90 backdrop-blur-sm transition-all duration-300 group-hover:border-sky-400/60 group-hover:-translate-y-0.5 group-hover:shadow-[0_18px_35px_rgba(0,0,0,0.65)] active:scale-[0.98]">
        {/* Hero media area */}
          ...
          {hasMultiple && (
            <>
              <button
                type="button"
                onClick={goPrev}
                className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-7 sm:h-7 rounded-full bg-black/60 border border-white/30 flex items-center justify-center text-sm sm:text-xs text-slate-100 opacity-0 group-hover:opacity-100 transition-opacity active:scale-95"
              ...
              <button
                type="button"
                onClick={goNext}
                className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-7 sm:h-7 rounded-full bg-black/60 border border-white/30 flex items-center justify-center text-sm sm:text-xs text-slate-100 opacity-0 group-hover:opacity-100 transition-opacity active:scale-95"
        ...
        <div className="p-3 sm:p-3.5 md:p-4">
          <div className="flex items-start justify-between mb-1.5">
            <h3 className="font-medium text-[15px] sm:text-sm md:text-[15px] line-clamp-1 group-hover:text-sky-200 transition-colors">
              {project.title}
            </h3>
            <span className="text-xs sm:text-[11px] text-slate-500 ml-2 flex-shrink-0">
              {project.year}
            </span>
          </div>
          ...
          <p className="text-xs sm:text-[11px] md:text-xs text-slate-400 line-clamp-2 mb-3 leading-relaxed">
            {project.description}
          </p>
...
function ProjectDetail({
  ...
  const mediaItems = React.useMemo<MediaItem[]>(() => {
    ...
    if (project.images?.length) {
      ...
      if (project.thumbnail)
        items.push({ type: "image", src: project.thumbnail });
    }
  ...
  }, [project.video, project.images, project.thumbnail]);
  ...
  return (
    <div
      className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm overflow-y-auto overscroll-contain"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
      style={{
    ...
    >
      <div className="min-h-full flex items-start justify-center p-2 sm:p-3 md:p-6">
        <div
          ...
          className="relative w-full max-w-6xl flex flex-col rounded-2xl sm:rounded-3xl border border-white/10 bg-[#05070c]/95 shadow-[0_28px_80px_rgba(0,0,0,0.85)] overflow-hidden my-2 sm:my-4"
        >
          {/* Sticky header */}
          <div className="sticky top-0 z-20 bg-[#05070c]/95 backdrop-blur-xl border-b border-white/10 px-3 sm:px-4 md:px-6 py-3 sm:py-4">
            <div className="flex items-start justify-between gap-3 sm:gap-4">
              <div className="min-w-0 flex-1">
                <h2 className="text-lg sm:text-xl md:text-2xl font-semibold text-slate-50 truncate">
                  {project.title}
                </h2>
              ...
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <button
                  onClick={onClose}
                  className="w-12 h-12 sm:w-10 sm:h-10 rounded-full border border-white/15 hover:border-white/40 hover:bg-white/5 active:scale-95 text-2xl sm:text-xl text-slate-300 flex items-center justify-center transition-transform"
                  aria-label="Close"
                >
                  ×
                </button>
          ...
          <div className="flex-1 overflow-y-auto px-3 sm:px-4 md:px-6 py-3 sm:py-4">
            <div className="grid grid-cols-1 lg:grid-cols-[420px,1fr] gap-4 md:gap-6">
              {/* MEDIA: main viewer + thumbnails */}
              <div className="space-y-3">
                <div className="rounded-2xl border border-white/10 bg-slate-900/40 overflow-hidden">
                  {/* Main media viewer */}
                  <div className="flex items-center justify-center h-[240px] sm:h-[280px] md:h-[320px] bg-gradient-to-br from-slate-800/40 to-slate-900/20">
                    {activeMedia?.type === "video" ? (
                      ...
                      <img
                        key={activeMedia?.src}
                        src={activeMedia?.src || project.thumbnail}
                        alt={project.title}
                        className="w-full h-full object-contain"
                        draggable={false}
                      />
                  ...
                  {/* Thumbnails rail */}
                  {mediaItems.length > 1 && (
                    <div className="border-t border-white/10 bg-[#05070c]/40">
                      <div className="flex lg:flex-col gap-2 p-2 overflow-x-auto lg:overflow-y-auto lg:max-h-[320px] scrollbar-thin">
                        {mediaItems.map((item, idx) => {
                          ...
                          return (
                            <button
                              ...
                              className={[
                                "relative shrink-0 rounded-xl overflow-hidden border transition-all active:scale-95",
                                active
                                  ? "border-sky-400/70 ring-2 ring-sky-400/20"
                                  : "border-white/10 hover:border-white/25 active:border-white/40",
                                "w-[90px] h-[60px] sm:w-[110px] sm:h-[68px] lg:w-full lg:h-[74px]",
                              ].join(" ")}
                            ...
                            >
                              {item.type === "video" ? (
                                <div className="w-full h-full bg-black flex items-center justify-center">
                                  ...
                                  <div className="absolute bottom-1 left-1 px-1.5 py-0.5 rounded-md bg-black/70 text-[10px] text-slate-100 border border-white/10">
                                    Video
                                  </div>
                                </div>
                              ) : (
          ...
          <div className="sticky bottom-0 z-20 lg:hidden bg-[#05070c]/95 backdrop-blur-xl border-t border-white/10 px-3 sm:px-4 py-3">
            <div className="flex items-center justify-between gap-3">
              <button
                onClick={onPrev}
                className="flex-1 px-4 py-3 rounded-xl bg-[#0b1018] border border-white/10 active:border-white/25 active:scale-95 text-sm font-medium text-slate-200 transition-transform"
              >
                ← Previous
              </button>
          ...
          <div className="pointer-events-none absolute inset-0 z-30">
            <button
              type="button"
              onClick={onPrev}
              className="pointer-events-auto hidden lg:flex absolute left-3 md:left-5 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/80 border border-white/20 hover:border-white/40 hover:bg-black/90 active:scale-95 items-center justify-center text-slate-100 text-2xl shadow-lg transition-transform"
            ...
            <button
              type="button"
              onClick={onNext}
              className="pointer-events-auto hidden lg:flex absolute right-3 md:right-5 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-black/80 border border-white/20 hover:border-white/40 hover:bg-black/90 active:scale-95 items-center justify-center text-slate-100 text-2xl shadow-lg transition-transform"
